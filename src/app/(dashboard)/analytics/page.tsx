...
import { Box, Card, CardContent, Typography, Grid, TextField, MenuItem, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper } from '@mui/material';
import { Download as DownloadIcon } from '@mui/icons-material';
import { BarChart, Bar, LineChart, Line, PieChart, Pie, Cell, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend } from 'recharts';
import DashboardLayout from '@/components/organisms/DashboardLayout';
import { mockAnalytics, formatCurrency, formatNumber } from '@/lib/mock-data';
    ...
    <DashboardLayout>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
        ...
        <Grid container spacing={2.5}>
          {[
            { label: 'Total Revenue', value: formatCurrency(mockAnalytics.reduce((s, d) => s + d.revenue, 0)) },
            { label: 'Total Visitors', value: formatNumber(mockAnalytics.reduce((s, d) => s + d.visitors, 0)) },
            { label: 'Avg Sessions/Day', value: formatNumber(Math.round(mockAnalytics.reduce((s, d) => s + d.sessions, 0) / mockAnalytics.length)) },
            { label: 'Total Conversions', value: formatNumber(mockAnalytics.reduce((s, d) => s + d.conversions, 0)) },
          ].map((kpi) => (
        ...
        <Card><CardContent sx={{ p: 2.5 }}>
          ...
          <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: '8px' }}>
            <Table size="small">
              ...
              <TableBody>
                {mockAnalytics.slice(-10).map((row) => (
